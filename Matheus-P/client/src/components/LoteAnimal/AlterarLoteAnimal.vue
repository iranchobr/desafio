...
export default {
  ...
  methods: {
    onSubmit (event) {
      ...
      const relacao = {
        fk_id_animal: this.form.fk_id_animal,
        fk_id_lote: this.form.fk_id_lote,
      ...
      console.log(relacao)
      axios.put('http://localhost:3000/animais_lotes/' + this.$route.query.id, relacao)
        .then(relacao => {
          alert('Relação Atualizada na Base de Dados!')
  ...
  async created () {
    ...
    try {
      this.relacao = await axios.get('http://localhost:3000/animais_lotes/' + this.$route.query.id)
      this.relacao = this.relacao.data

      this.form.fk_id_animal = this.relacao.fk_id_animal
      this.form.fk_id_lote = this.relacao.fk_id_lote
      this.form.dt_entrada = this.relacao.dt_entrada
      this.form.dt_saida = this.relacao.dt_saida
      this.form.dt_ultima_movimentacao = this.relacao.dt_ultima_movimentacao
      this.relacao.ic_bezerro ? this.form.ic_bezerro = 1 : this.form.ic_bezerro = 0
    } catch (err) {
      console.log(err)
    }
  }
