...
export default {
  ...
  created() {
    ...
    this.$http.get('http://localhost:3333/pessoas/animais/'+this.$route.query.id)
      .then(res => res.json())
      .then(animais => this.animais = animais);

    this.$http.get('http://localhost:3333/pessoas/'+this.$route.query.id)
      .then(res => res.json())
      .then(user => {
          this.name_user = user.no_pessoa;
      });
  }
}
</script>
